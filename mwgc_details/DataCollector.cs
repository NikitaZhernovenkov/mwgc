...
namespace mwgc
{
  public class DataCollector
  {
    ...
    {
      string option = Compiler.Options["xname"];
      return (string.IsNullOrEmpty(option) ? precompileName : option + "_" + precompileName).ToUpper(new CultureInfo("en-US", false));
    }

    private string ResolveRealName(string precompileName)
    {
      string upper = Compiler.Options["xname"].ToUpper(new CultureInfo("en-US", false));
      string str = precompileName;
      if (precompileName.StartsWith("x_"))
        str = upper + precompileName.Substring(1);
      return str.ToUpper(new CultureInfo("en-US", false));
    }

    public RealGeometryFile Collect(RawGeometryFile rawGeom)
    {
      // ISSUE: The method is too long to display (79250 instructions)
    }
    ...
    public RealGeometryFile Collect(AseFile aseFile)
    {
      ...
      for (int index3 = 0; index3 < aseFile.ObjectCount; ++index3)
      {
        ...
        {
          string str = aseFile[index3].Name.Substring(1).ToUpper(new CultureInfo("en-US", false));
          if (str.IndexOf("[") > -1)
        ...
        else
        {
          ...
          RealGeometryPart part = new RealGeometryPart();
          if (aseFile[index3].Name.ToUpper(new CultureInfo("en-US", false)).StartsWith("BASE_"))

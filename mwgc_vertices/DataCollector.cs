...
namespace mwgc
{
    public class DataCollector
    {
        ...
        public RealGeometryFile Collect(RawGeometryFile rawGeom)
        {
            ...
            Dictionary<string, ArrayList> mount_points = new Dictionary<string, ArrayList>();
            //Dictionary<string, ArrayList> mount_points = new Dictionary<string, ArrayList>();
            ...
            {
                if (rawGeom.Header.ObjHeaders[index1].ObjName.Data.Contains("#"))
                { 
                    string detail_name = rawGeom.Header.ObjHeaders[index1].ObjName.Data.Split('#')[0];
                    ...
                    if(mount_points.Keys.Contains(detail_name))
                    {
                        mount_points[detail_name].Add(realMountPoint);
                    }
                    else
                    {
                        mount_points.Add(detail_name, new ArrayList() { realMountPoint });
                    }
                }
                else
                {
